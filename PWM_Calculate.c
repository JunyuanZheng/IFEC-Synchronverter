...
float32 P_set,Tm,T_sum,Te,Te_1,Wr,W,Dp,J; //P环参数
...
void PIfunc_calc(PI_FUNC*p)
{
	...
	a0=p->Kp*(1+p->T/p->Ti);
	a1=p->Kp*;
	p->Output=p->LastOutput+a0*p->ek-a1*p->ek_1;
#else
	p->Output=p->LastOutput+p->a0*p->ek-p->a1*p->ek_1;
#endif
